...
class AudioReactiveCubesScene {
  ...
  setupScene() {
    ...
    // Load the cube model once and clone for both sides
    loader.load("./models/cubeBigger.gltf", (cubeModel) => {
      ...
      (cubeMesh.material as any).color = cubeColor.clone();
      (cubeMesh.material as any).dithering = true;
      (cubeMesh.material as any).flatShading = false;
      cubeMesh.position.set(-6.75, -29.3, 0);
      cubeMesh.rotateX(0.075);
      cubeMesh.rotateY(-0.1);
      this.leftCube = cubeMesh;
      this.scene.add(cubeMesh);

      // Clone for the right cube
      const rightMesh = cubeMesh.clone() as Mesh;
      (rightMesh.material as any) = (cubeMesh.material as any).clone();
      (rightMesh.material as any).color = cubeColor.clone();
      rightMesh.position.set(6.75, -29.7, 0);
      rightMesh.rotation.set(0, 0, 0);
      rightMesh.rotateX(0.075);
      rightMesh.rotateY(0.075);
      this.rightCube = rightMesh;
  ...
  animate() {
    ...
    // Only animate the cubes when audio is playing
