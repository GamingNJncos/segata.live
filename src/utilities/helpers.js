...
export const sceneWidth = () => {
  ...
  const width = idealWidth > actualWidth ? actualWidth : idealWidth;

  // Only update the CSS custom property when the value actually changes
  if (width !== _cachedSceneWidth) {
...
export const getRandomPositionsNotInCenter = (w, h) => {
  // Generate random position in valid range directly, avoiding center (-30, 30)
  const randomOutsideCenter = (max) => {
    // Generate in range [30, max], then randomly negate
    const val = Math.random() * (max - 30) + 30;
    return Math.random() < 0.5 ? val : -val;
  };
  return [randomOutsideCenter(w), randomOutsideCenter(h)];
};
